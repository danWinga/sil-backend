...
class CategoryViewSet(viewsets.ModelViewSet):
    ...
    def average_price(self, request, pk=None):
        ...
        result    = Product.objects.filter(categories__in=descendants)\
                                   .aggregate(avg_price=Avg('price'))
        avg       = result['avg_price'] or 0
        return Response({
            'category': cat.id,
            'average_price': float(round(avg, 2))
        })
...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        data      = (self.get_serializer(instances, many=many).data
                     if many else self.get_serializer(instances).data)
